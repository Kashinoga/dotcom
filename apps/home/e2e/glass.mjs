...
for (const ui of ['flat', 'bubble']) {
	...
	for (const path of ['/about', '/apps/air-traffic']) {
		...
		if (ui === 'flat')
			ok(`flat: ${path} no backdrop-filter`, blurs.length === 0, blurs.slice(0, 2).join(' | '));
		...
		const bd = await p.locator('.surface-backdrop').evaluate((e) => {
			const s = getComputedStyle(e);
			return { bg: s.backgroundColor, bi: s.backgroundImage.slice(0, 30) };
		});
		if (ui === 'flat')
			ok(
				`flat: ${path} backdrop is glass (translucent, no live filter)`,
				/rgba|\/ 0?\.\d+\)/.test(bd.bg) && bd.bi === 'none',
				JSON.stringify(bd)
			);
		...
		// light — see the .surface-backdrop bubble rule); what still separates Bubble is the blur.
			...
			ok(
				`bubble: ${path} backdrop translucent, no sheen`,
