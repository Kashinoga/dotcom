...
after = await readAll();
ok('remove drops the right phrase', after.length === 3 && !after.includes('A route map of my internet'), JSON.stringify(after));
ok('final order', JSON.stringify(after) === JSON.stringify(['Mind the gap', 'Publicly available data, made good to look at', 'Ad astra']), JSON.stringify(after));

// ── disabled edges ──────────────────────────────────────────────────────────
ok('first row cannot move up', await page.getByRole('button', { name: 'Move phrase 1 up' }).isDisabled());
ok('last row cannot move down', await page.getByRole('button', { name: 'Move phrase 3 down' }).isDisabled());
...
ok('kept phrase written 4×', count('>Publicly available data, made good to look at<') === 4, `${count('>Publicly available data, made good to look at<')}`);
ok('deleted phrase is gone', count('A route map of my internet') === 0, `${count('A route map of my internet')}`);
ok('removed old phrase "Take care" gone', count('>Take care<') === 0);

// structure preserved
ok('two ticker-group divs remain', count('class="ticker-group"') === 2, `${count('class="ticker-group"')}`);
...
ok('item count is phrases × repeat × groups', count('class="ticker-item"') === 3 * 2 * 2, `${count('class="ticker-item"')}`);
