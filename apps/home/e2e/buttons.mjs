...
for (const ui of ['pixelite', 'aeropalite']) {
	...
	for (const [sel, label] of [
		['button.tb', 'PB New (.tb)'],
		['.pb-head button.icon-btn', 'PB Back (.icon-btn)'],
		['button.mini', 'PB .mini']
	]) {
		...
		ok(
			`${ui}: ${label} transitions transform on the spring`,
			(await easingOf(loc)).includes('0.34'),
			await easingOf(loc)
		);
		...
		ok(
			...
			(await scaleOf(loc)) === HOVER,
			String(await scaleOf(loc))
		);
		await p.mouse.down();
		await p.waitForTimeout(220);
		ok(
			`${ui}: ${label} press = ${PRESS}`,
			(await scaleOf(loc)) === PRESS,
			String(await scaleOf(loc))
		);
	...
	for (const [sel, label] of [
		['button.icon-btn.back', 'Panel Back'],
		['button.seg', 'Settings .seg'],
		['button.sky-opt', 'Settings .sky-opt'],
		['a.chip', 'Panel .chip']
	]) {
	...
	await p.waitForTimeout(450);
	ok(
		`${ui}: ATFC .field hover = ${HOVER}`,
		(await scaleOf(fld)) === HOVER,
		String(await scaleOf(fld))
	);
	...
	for (const [sel, label, hov, prs] of [
		...
		await p.goto(B + PUD, { waitUntil: 'networkidle' });
		...
		if (sel === '.pud-reset') {
			await p.locator('[data-pud-settings]').first().click();
			await p.waitForTimeout(400);
		}
		const loc = p.locator(sel).first();
		if (!(await loc.count())) {
			ok(`${ui}: ${label} is present`, false, 'absent');
			continue;
		}
		await loc.scrollIntoViewIfNeeded().catch(() => {});
		await loc.hover({ force: true });
		...
		ok(
			`${ui}: ${label} rides the shared spring`,
			/cubic-bezier\(0\.34, 1\.4/.test(await easingOf(loc)),
			await easingOf(loc)
		);
		await p.mouse.move(5, 5);
		await p.mouse.up();
...
const peakOnTap = async (p, sel) => {
	...
	await p.mouse.down();
	await p.mouse.up(); // a TAP: no hold at all
	await p.waitForTimeout(400);
	const peak = await p.evaluate(() => {
		window.__watching = false;
		return +window.__peak.toFixed(3);
	});
	await p.mouse.move(700, 930);
...
for (const ui of ['pixelite', 'aeropalite']) {
	...
	ok(
		`${ui}: tapping an Apps card squashes it to its own softer amount`,
		card <= 0.991 && card > 0.96,
		String(card)
	);
	await ctx.close();
}
...
// must not even mark the control.
{
	...
	ok(
		'reduced-motion: no .btn-tap marked',
		await p.evaluate(() => {
...
// A dead control stays dead — the floor must not revive a disabled button.
{
	...
	await p.goto(B + '/apps/intergalactic-park-ranger', { waitUntil: 'networkidle' });
	await p.waitForTimeout(2000);
	ok(
		'a disabled control takes no tap squash',
		await p.evaluate(() => {
...
// reduced motion: no scale anywhere
{
	...
	await p.waitForTimeout(400);
	ok(
		'reduced-motion: Back does not scale on hover',
		(await scaleOf(loc)) === 1,
		String(await scaleOf(loc))
	);
	await ctx.close();
