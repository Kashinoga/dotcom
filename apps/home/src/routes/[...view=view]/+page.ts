...
import { resolveRange, rangeToken, resolveRefresh, refreshToken } from '$lib/scope';
import type { PageLoad } from './$types';

/** Only the Air Traffic board reads `?field=`, `?range=` and `?refresh=`. */
...
export const load: PageLoad = ({ params, url }) => {
	if (!params.view) return { view: null, field: null, range: null, refresh: null };

	const view = slugToView(params.view)!;

	// The board's three controls, each mirrored into the query so a link restores the exact
	...
	return { view, field: field ? field.iata : null, range, refresh };
};
