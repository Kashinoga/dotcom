...
<style>
	...
	/* The full-viewport panel (.surface.expanded .settings) and the Pixelite docs sheet
	   (.docs-settings) share one grid: settings has more width than a single column of controls
	   needs, so the groups flow into as many ~19rem tracks as fit (two or three on a wide desktop).
	   Grid, not multicol: a group is one cell, so it can never split across a column the way
	   multicol's break rules let a note orphan. Both collapse to one track on a narrow measure. */
	...
	/* The panel levels leads to a two-line floor so controls line up across a grid row even when
	   ...
	   its groups and tightens the lead→control step below instead). */
	.surface.expanded .settings .stg-group > :first-child {
		min-height: 3.25rem;
	}
	/* Docs grid: groups top-align in their row (align-items: start) so single-line leads keep their
	   controls level without the panel's min-height floor, and each control sits close under its
	   lead — a small section label — rather than at the group's default 1.05rem step. */
	.docs-settings {
		align-items: start;
	}
	.docs-settings .stg-group > .seg-lead + * {
	...
	.docs-settings .stg-group + .stg-group {
		margin-top: 0;
	}
	...
	/* The Apps docs page (.docs-sheet.app-page): a full-width sheet, so the shelf of cards fills it
	   as a responsive grid. The panel's two-column split (appCards → two .app-cards lists) is
	   flattened with display:contents, so every card becomes a direct grid item of .app-cols and
	   the column count follows purely from how many ~17rem tracks the viewport fits — one on a
	   phone, up to three or four on a wide desktop. align-items:start lets each card keep its
	   natural height. The short intro prose above stays at a readable measure. */
	.app-page .docs-prose {
		max-width: none;
	}
	...
	.app-page .docs-prose > :not(.app-cols):not(blockquote) {
		max-width: 72ch;
	}
	.app-page .app-cols {
		display: grid;
